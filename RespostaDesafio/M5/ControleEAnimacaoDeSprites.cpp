...
int main()
{
    ...
    GLuint idleTexID = loadTexture("../assets/sprites/Vampires1_Idle_full.png", imgWidth, imgHeight);
    ...
    vampirao.position = vec3(400.0, 120.0, 0.0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        // Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
        {
            ...
            if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
            {
                ...
                char tmp[256];
                sprintf(tmp, "Ola Vampirinho! -- Felipe\tFPS %.2lf", fps);
        ...
        if (isMoving)
        {
            vampirao.texID = texID;
            vampirao.nFrames = 6;
            vampirao.ds = 1.0f / 6.0f;
            vampirao.VAO = setupSprite(vampirao.nAnimations, vampirao.nFrames, vampirao.ds, vampirao.dt);
        }
        else
        {
            vampirao.texID = idleTexID;
            vampirao.nFrames = 4;
            vampirao.ds = 1.0f / 4.0f;
            vampirao.VAO = setupSprite(vampirao.nAnimations, vampirao.nFrames, vampirao.ds, vampirao.dt);
        }
        ...
        if ((deltaT >= 1.0 / FPS) && isMoving && xDirection)
        {
            background.iFrame = (background.iFrame + xDirection) % 100;
        }
        ...
        vec2 offsetTex;
        // horizontal: coluna do frame
        offsetTex.s = vampirao.iFrame * vampirao.ds;

        // vertical: linha da animação
