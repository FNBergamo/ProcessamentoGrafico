...
int main()
{
    ...
    cout << endl;
    cout << "Tentativas: " << tryCount << " | Eliminados: " << 0 << " | Pontos nessa rodada: " << points << " | Pontuacao total: " << totalPoints << endl;
    ...
    while (!glfwWindowShouldClose(window))
    {
        if (todosEliminados())
        {
            cout << "Fim de jogo! Pontuacao total: " << totalPoints << endl;
            char resposta;
...
void mouse_button_callback(GLFWwindow *window, int button, int action, int mods)
{
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
    {
        ...
        int colunaClicada = xpos / QUAD_WIDTH;
        int linhaClicada = ypos / QUAD_HEIGHT;
        grid[linhaClicada][colunaClicada].eliminated = true;
        iSelected = colunaClicada + linhaClicada * COLS; // indice linear do quadrado selecionado
    }
}
...
void eliminarSimilares(float tolerancia)
{
    ...
    vec3 selectedColor = grid[y][x].color;
    ...
    for (int i = 0; i < ROWS; i++)
    {
        for (int j = 0; j < COLS; j++)
        {
            if (!grid[i][j].eliminated)
            {
                vec3 actualColor = grid[i][j].color;

                float d = sqrt(pow(selectedColor.r - actualColor.r, 2) + pow(selectedColor.g - actualColor.g, 2) + pow(selectedColor.b - actualColor.b, 2));
                float dd = d / dMax;
    ...
    points = eliminatedCount * std::max(10 - tryCount, 1);
    totalPoints += points;

    cout << "Tentativas: " << tryCount + 1 << " | Eliminados: " << eliminatedCount << " | Pontos nessa rodada: " << points << " | Pontuacao total: " << totalPoints << endl;
